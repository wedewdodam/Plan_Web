    ...
    public partial class Index
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (authState.User.Identity.IsAuthenticated)
            {
                ...
                //await base.OnInitializedAsync(firstRender);
            }
            else
            {
        ...
        private async Task OnByEditA(Join_Article_Cycle_Cost_EntityA ar)
        {
            ...
            dnn.Set_Repair_Rate = Convert.ToInt32(ar.Repair_Rate);
        ...
        private async Task btnSave()
        {
            ...
            else
            {
                ...
                {
                    dnn.Law_Repair_Cycle_All = art.All_Cycle;
                    dnn.Repair_Last_Year_All = 0;
                    dnn.Repair_Plan_Year_All = 0;
                    dnn.Set_Repair_Cycle_All = 0;
                ...
                if (intPartAllCount < 1)
                {
                    ...
                    dnn.Set_Repair_Rate = 0;
                    dnn.Set_Repair_Cycle_Part = 0;
                    dnn.Repair_Last_Year_Part = 0;
                ...
                else
                {
                    dnn.Law_Repair_Cycle_Part = art.Part_Cycle;
                    dnn.Law_Repair_Rate = Convert.ToInt32(art.Repair_Rate);
                    dnn.Set_Repair_Rate = Convert.ToInt32(art.Repair_Rate);
                ...
                dnn.Part_Cycle_Num = intPartAllCount;
        ...
        private async Task OnChageingAll(ChangeEventArgs a)
        {
            ...
            if ((Z - DateTime.Now.Year) >= 0)
            {
                ...
                dnn.Repair_Plan_Year_All = art.Installation.Year + dnn.Set_Repair_Cycle_All;
                Cycle_all();
                if (dnn.Set_Repair_Rate > 0 || dnn.Set_Repair_Cycle_Part > 1)
                {
                    Cycle_PartA();
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task btnAllCycle()
        {
            if (strSortA != null || strSortA != "" || strSortA == "Z")
            {
                rae = await article_Lib.GetLIst_RepairArticle_Sort(rpn.Repair_Plan_Code, "Sort_A_Code", strSortA, Apt_Code);
                foreach (var st in rae)
                {
                    string strAid = st.Aid.ToString();
                    string strArticleName = st.Repair_Article_Name;
                    await OnCycleAllInsert(strAid, strArticleName, st.Sort_C_Code);
                }
        ...
        /// </summary>
        /// <returns></returns>
        LogsView_Entity logs { get; set; } = new LogsView_Entity();
        private async Task OnCycleAllInsert(string ArticleCode, string ArticleName, string SortCode)
        {

            string strAgo_plan_code = await repair_Plan_Lib.BeComplete_Code(Apt_Code); // 마지막 조정이 완료된 정기조정의 코드 불러오기
            int intArticle = await cycle_Lib.Being_Detail_Cycle_Article(strAgo_plan_code, SortCode, ArticleName);
            if (intArticle > 0)
            {
                //Repair_Plan_Entity.Join_Article_Cycle_Cost_Entity vnn = (new Repair_Plan_Lib.Repair_Cycle()).Detail_Cycle_Article(strAgo_plan_code, strSort_C_Code, strArticle_Name);
                var vnn = await cycle_Lib.Detail_Cycle_Article(strAgo_plan_code, SortCode, ArticleName);
                Cycle_Entity cnn = new Cycle_Entity();

                cnn.Apt_Code = Apt_Code;
                cnn.Repair_Plan_Code = Aid;
                cnn.Repair_Article_Code = ArticleCode;
                cnn.Division = vnn.Division;
                cnn.Sort_A_Code = vnn.Sort_A_Code;
                cnn.Sort_B_Code = vnn.Sort_B_Code;
                cnn.Sort_C_Code = vnn.Sort_C_Code;
                cnn.Sort_A_Name = vnn.Sort_A_Name;
                cnn.Sort_B_Name = vnn.Sort_B_Name;
                cnn.Sort_C_Name = vnn.Sort_C_Name;
                cnn.Law_Repair_Cycle_All = vnn.Law_Repair_Cycle_All;
                cnn.Set_Repair_Cycle_All = vnn.Set_Repair_Cycle_All;
                cnn.Law_Repair_Cycle_Part = vnn.Law_Repair_Cycle_Part;
                cnn.Set_Repair_Cycle_Part = vnn.Set_Repair_Cycle_Part;
                cnn.Law_Repair_Rate = vnn.Law_Repair_Rate;
                cnn.Set_Repair_Rate = vnn.Set_Repair_Rate;
                cnn.Repair_Last_Year_Part = vnn.Repair_Last_Year_Part;
                cnn.Repair_Last_Year_All = vnn.Repair_Last_Year_Part;
                cnn.Repair_Plan_Year_All = vnn.Repair_Plan_Year_All;
                cnn.Repair_Plan_Year_Part = vnn.Repair_Plan_Year_Part;
                cnn.All_Cycle_Num = vnn.All_Cycle_Num;
                cnn.Part_Cycle_Num = vnn.Part_Cycle_Num;
                cnn.Repair_Cycle_Etc = vnn.Repair_Cycle_Etc;
                cnn.User_ID = User_Code;
                #region 아이피 입력
                string myIPAddress = "";
                var ipentry = System.Net.Dns.GetHostEntry(System.Net.Dns.GetHostName());
                foreach (var ip in ipentry.AddressList)
                ...
                cnn.Post_IP = myIPAddress;
                #endregion 아이피 입력
                //Repair_Plan_Lib.Repair_Article art = new Repair_Plan_Lib.Repair_Article();
                int intBeing = await cycle_Lib.Being_Article_Cycle(Apt_Code, Aid, ArticleCode);
                //Repair_Plan_Lib.Repair_Cycle dnn = new Repair_Plan_Lib.Repair_Cycle();
                if (intBeing < 1)
                {
                    int now_year = DateTime.Now.Year;
                    if (cnn.Repair_Plan_Year_All >= now_year || cnn.All_Cycle_Num == 0)
                    {
                        if (cnn.Repair_Plan_Year_Part >= now_year || cnn.Part_Cycle_Num == 0)
                        {
                            if (cnn.Part_Cycle_Num > 0 || cnn.All_Cycle_Num > 0)
                            {
                                await cycle_Lib.Add_RepairCycle(cnn);//수선항목에 해당 수선주기 존재 여부
                                                                     //art.Update_Cycle_Count_Add(vnn.Aid.ToString()); //수선항목에 해당 수선주기 존재 여부
                                                                     //(new Repair_Plan_Lib.Repair_Article()).Update_Cycle_Count_Add(vnn.Aid.ToString());
                                await article_Lib.Update_Cycle_Count_Add(Convert.ToInt32(cnn.Repair_Article_Code));

                                logs.Application = "수선주기 조정 저장";
                                logs.Message = cnn.Repair_Article_Code + "의 수선주기를 일괄 저장 하였습니다.";
                                logs.Logger = User_Name;
                                logs.IpAddress = cnn.Post_IP;
                                await logView_Lib.Add(logs); 
                            }
                        }
                    }
                }
