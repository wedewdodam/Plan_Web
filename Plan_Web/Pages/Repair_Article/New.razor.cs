...
namespace Plan_Web.Pages.Repair_Article
{
    public partial class New
    {
        ...
        private async Task onSortA(ChangeEventArgs a)
        {
            ...
            fnn = await facility_Sort_Lib.GetList_Sort_A_List(strSortA);
        }
        ...
        private async Task onSortB(ChangeEventArgs a)
        {
            ...
            fnn = await facility_Sort_Lib.GetList_FacilitySortA(strSortB);
        }
        ...
        private async Task OnYearA(ChangeEventArgs a)
        {
            ...
            if (intD < 0)
            {
                strInsYearA = rpn.Founding_Date.Year.ToString();
        ...
        private async Task OnYearB(ChangeEventArgs a)
        {
            ...
            if (intD < 0)
            {
                strInsYearB = rpn.Founding_Date.Year.ToString();
        ...
        private async Task btnSave()
        {
            ...
            {
                await JSRuntime.InvokeVoidAsync("exampleJsFunctions.ShowMsg", "단위를 입력하지 않았습니다.");
            }
            else if (bnn.Installation.Year < rpn.Founding_Date.Year)
            {
                await JSRuntime.InvokeVoidAsync("exampleJsFunctions.ShowMsg", "전체수선 최종수선년도가 사용검사년도 보다 앞설 수는 없습니다.");
            }
            else if (bnn.Installation.Year > (rpn.Founding_Date.Year + rpn.Plan_Period))
            {
                await JSRuntime.InvokeVoidAsync("exampleJsFunctions.ShowMsg", "전체수선 최종수선년도가 전체 기획기간을 넘을 수는 없습니다.");
            }
            else if (bnn.Installation_Part.Year < rpn.Founding_Date.Year)
            {
                await JSRuntime.InvokeVoidAsync("exampleJsFunctions.ShowMsg", "부분수선 최종수선년도가 사용검사년도 보다 앞설 수는 없습니다.");
            }
            else if (bnn.Installation_Part.Year > (rpn.Founding_Date.Year + rpn.Plan_Period))
            {
                await JSRuntime.InvokeVoidAsync("exampleJsFunctions.ShowMsg", "부부수선 최종수선년도가 전체 기획기간을 넘을 수는 없습니다.");
            }
            else if (intB > intA)
            {
