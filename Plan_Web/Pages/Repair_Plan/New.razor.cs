...
namespace Plan_Web.Pages.Repair_Plan
{
    public partial class New
    {
        ...
        private AptInfor_Entity aif { get; set; } = new AptInfor_Entity();
        //List<Plan_Review_Plan_Entity> rnnA { get; set; } = new List<Plan_Review_Plan_Entity>();
        ...
        {
            intCode = await repair_Plan_Lib.Being_Count(Apt_Code);
            if (intCode > 0)
            {
        ...
        {
            InsertViews = "B";
            aif = await aptInfor_Lib.Detail_Apt(Apt_Code);
            rpn.Apt_Code = Apt_Code;
            rpn.Adjustment_Man = User_Name;
            rpn.Founding_Date = aif.AcceptancedOfWork_Date;
            //rpn.Founding_man = aif.
            rpn.Founding_Date = Convert.ToDateTime(BuildDate);
        ...
        private void btnEdit(Repair_Plan_Entity aq)
        {
            InsertViews = "B";
            rpn = aq;
        ...
        public async Task btnSave()
        {
            ...
            else
            {
                if (rpn.Aid < 1)
                {
                    ...
                    #endregion 아이피 입력
                    rpn.LastAdjustment_Date = rpn.Founding_Date;
                    ...
                    rpn.Repair_Plan_Code = Apt_Code + (await repair_Plan_Lib.Last_Number_Apt(Apt_Code)).ToString();
