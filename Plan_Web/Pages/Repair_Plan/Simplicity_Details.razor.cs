...
{
    public partial class Simplicity_Details
    {
        #region 인스턴스
        ...
        List<Article_Entity> annlist { get; set; } = new List<Article_Entity>();
        List<Article_Entity> annAgolist { get; set; } = new List<Article_Entity>();
        List<Cycle_Entity> bnnlist { get; set; } = new List<Cycle_Entity>();
        List<Cycle_Entity> bnnAgolist { get; set; } = new List<Cycle_Entity>();
        List<Cost_Entity> cnnlist { get; set; } = new List<Cost_Entity>();
        List<Cost_Entity> cnnAgolist { get; set; } = new List<Cost_Entity>();
        ...
        public string Edit_Views { get; set; } = "A"; // 수정 여부
        //public string InsertViews { get; set; } = "A";
        public string Article_Edit { get; set; } = "A";
        public string Cycle_Edit { get; set; } = "A";
        public string Cost_Edit { get; set; } = "A";
        public string Cost_EditA { get; set; } = "A";
        ...
        public int Total_Article { get; set; } = 0;
        ...
        private async Task DetailsView()
        {
            ...
            string Ago_Plan_Code = await repair_Plan_Lib.Last_Apt_Code(Apt_Code);

            annAgolist = await article_Lib.GetList_Article_Ago(Apt_Code, rpn.Repair_Plan_Code, Ago_Plan_Code);
            annlist = await article_Lib.GetLIst_RepairArticle(Apt_Code, Aid);

            

            bnnlist = await cycle_Lib.GetList_Cycle(Apt_Code, Aid);
            bnnAgolist = await cycle_Lib.GetList_Cycle_Ago(Apt_Code, rpn.Repair_Plan_Code, Ago_Plan_Code);

            cnnlist = await cost_Lib.GetList(Apt_Code, Aid);
            cnnAgolist = await cost_Lib.GetList_Cost_Ago(Apt_Code, Aid, Ago_Plan_Code);

            Total_Article = await article_Lib.Article_Count(Aid);
        }
        ...
        /// 총론 부분 수정 열기
        /// </summary>
        private void btnEdit(Repair_Plan_Entity at)
        {
            pnn = at;
            strTitle = "총론부분 수정";
            Edit_Views = "B";
        ...
        private async Task btnRemove(string Code)
        {
            ...
            if (isDelete)
            {
                await repair_Plan_Lib.Delete_RepairPlan_PlanCode(Code); //장기수선계획 삭제
                ...
                MyNav.NavigateTo("/Repair_Plan/Simplicity_Plan");
            }
        }
        ...
        /// 수선항목 수정 열기
        /// </summary>
        private void btnEditArticle(Article_Entity at)
        {
            ann = at;
            strInsYearA = at.Installation.Year.ToString();
            strInsYearB = at.Installation_Part.Year.ToString();
            Article_Edit = "B";
        }
        ...
        private async Task btnSaveArticle()
        {
            ...
            //ann.Repair_Plan_Code = at.Repair_Plan_Code;
            #region 아이피 입력
            string myIPAddress = "";
            var ipentry = System.Net.Dns.GetHostEntry(System.Net.Dns.GetHostName());
            foreach (var ip in ipentry.AddressList)
        ...
        private async Task btnRemoveArticle(Article_Entity at)
        {
            bool isDelete = await JSRuntime.InvokeAsync<bool>("confirm", $"{at.Repair_Article_Name}을 정말로 삭제하시겠습니까?");
            if (isDelete)
            {
                await article_Lib.Remove_Repair_Article(at.Aid);
                await cycle_Lib.Delete_Article_Cycle(at.Repair_Plan_Code, at.Aid.ToString());
                await cost_Lib.Remove_Article_Cost(at.Repair_Plan_Code, at.Aid.ToString());
                await DetailsView();
            }
        }
        ...
        /// 수선주기 수정 열기
        /// </summary>
        private async Task btnEditCycle(Cycle_Entity at)
        {
            bnn = at;
            art = await article_Lib.Details_Repair_Article_Cycle_Sort_Review(at.Repair_Plan_Code, at.Repair_Article_Code, Apt_Code);
            intPartAllCount = at.Part_Cycle_Num;
            intAllCount = at.All_Cycle_Num;
            if (intPartAllCount > 0)
            {
                if (bnn.Set_Repair_Cycle_All > 0 && bnn.Set_Repair_Cycle_Part >= 0)
                {
                    if (bnn.Repair_Plan_Year_All > bnn.Repair_Plan_Year_Part)
                    {
                        int z = ((bnn.Repair_Plan_Year_All - bnn.Repair_Plan_Year_Part) % bnn.Set_Repair_Cycle_Part);
                        if (z > 0)
                        {
                            intPartCount = ((bnn.Repair_Plan_Year_All - bnn.Repair_Plan_Year_Part) / bnn.Set_Repair_Cycle_Part) + 1;
                            Part_Cycle_Schedule = art.Installation_Part.Year;
                        }
                        else
                        {
                            intPartCount = ((bnn.Repair_Plan_Year_All - bnn.Repair_Plan_Year_Part) / bnn.Set_Repair_Cycle_Part);
                            Part_Cycle_Schedule = art.Installation_Part.Year;
                        }
                    }
                    else if (bnn.Repair_Plan_Year_All < bnn.Repair_Plan_Year_Part)
                    {
                        int z = (bnn.Law_Repair_Cycle_All % bnn.Set_Repair_Cycle_Part);
                        if (z > 0)
                        {
                            intPartCount = ((bnn.Repair_Plan_Year_All + bnn.Law_Repair_Cycle_All) / bnn.Set_Repair_Cycle_Part);
                            Part_Cycle_Schedule = bnn.Repair_Plan_Year_All;
                        }
                        else
                        {
                            intPartCount = ((bnn.Repair_Plan_Year_All + bnn.Law_Repair_Cycle_All) / bnn.Set_Repair_Cycle_Part) - 1;
                            Part_Cycle_Schedule = bnn.Repair_Plan_Year_All;
                        }
                    }
                    else
                ...
                else if (bnn.Set_Repair_Cycle_All <= 0 && bnn.Set_Repair_Cycle_Part > 0)
                {
                    intPartCount = bnn.Part_Cycle_Num;
                }
            }
            else
            {
            ...
            strInsYearA = art.Installation.Year.ToString();
            strInsYearB = art.Installation_Part.Year.ToString();
            strTitle = "수선항목 수정";
        ...
        /// 입력된 수선주기 삭제
        /// </summary>
        private async Task btnRemoveCycle(Cycle_Entity at)
        {           
            bool isDelete = await JSRuntime.InvokeAsync<bool>("confirm", $"{at.Repair_Article_Name}을 정말로 삭제하시겠습니까?");
            if (isDelete)
            {
                await cycle_Lib.Delete_RepairCycle(at.Aid);
                await article_Lib.Update_Cycle_Count_Minus(Convert.ToInt32(at.Repair_Article_Code));
                await cost_Lib.Remove_Article_Cost(at.Repair_Plan_Code, at.Repair_Article_Code);
                await article_Lib.Update_Cost_Count_Minus(Convert.ToInt32(at.Repair_Article_Code));
                await DetailsView();
            }
        }
        ...
        private async Task OnChageingAll(ChangeEventArgs a)
        {
            ...
            if ((Z - DateTime.Now.Year) >= 0)
            {
                ...
                bnn.Repair_Plan_Year_All = art.Installation.Year + bnn.Set_Repair_Cycle_All;
                Cycle_all();
                if (bnn.Set_Repair_Rate > 0 || bnn.Set_Repair_Cycle_Part > 1)
                {
                    Cycle_PartA();
                }
            }
        ...
        private async Task btnEditCost(Cost_Entity at)
        {
            cnn = at;
            art = await article_Lib.Details_Repair_Article_Cycle_Sort_Review(at.Repair_Plan_Code, at.Repair_Article_Code, Apt_Code);           
            ...
            strSortA = cnn.Sort_A_Code;
            fnnB = await facility_Sort_Lib.GetList_FacilitySortA(strSortA);
            strSortB = cnn.Sort_B_Code;

            cnn.Staff_Code = User_Code;

            strCostA = "C";
            CostViewsA = "C";
            intPrice = cnn.Repair_All_Cost / art.Repair_Amount;
            Cost_Edit = "B";
            EditViews = "B";
        }
        ...
        private async Task OnBySelectAsync(Join_Article_Cycle_Cost_Entity ae)
        {
            ...
            try
            {
                ...
                cnn.Repair_Part_Cost = Convert.ToInt64(cnn.Repair_All_Cost * (art.Repair_Rate / 100));
                intPrice = (cnn.Repair_All_Cost / art.Amount);
            }
            catch (Exception)
            {
                cnn.Repair_All_Cost = 0;
            ...
            cnn.Apt_Code = art.Apt_Code;
            cnn.Repair_Article_Code = art.Repair_Article_Code;
            cnn.Repair_Plan_Code = Aid;
            cnn.Repair_Rate = art.Set_Repair_Rate;
            var fac = await facility_Sort_Lib.Detail_Sort(ae.Sort_C_Code);

            cnn.Sort_A_Code = art.Sort_A_Code;
            cnn.Sort_A_Name = art.Sort_A_Name;
            cnn.Sort_B_Code = art.Sort_B_Code;
            cnn.Sort_B_Name = art.Sort_B_Name;
            cnn.Sort_C_Code = art.Sort_C_Code;
            cnn.Sort_C_Name = art.Sort_C_Name;

            cnn.Staff_Code = User_Code;
            cnn.Repair_Amount = art.Amount;
            strCostA = "C";
            CostViewsA = "C";
        }
        ...
        private async Task btnSaveCostA()
        {
            ...
            else
            {
                if (pse.Repair_Amount > 0)
                {
                    ...
                    #region 수선금액 만들기
                    int intCount = await cost_Lib.Cost_Count(Aid, cnn.Repair_Article_Code); //해당 수선항목에 수선금액 존재 여부 입력
                    ...
                    if (intCount < 1)
                    {
                        ...
                        Personal_Price = 0;
                        //cnn = await cost_Lib.Detail_Cost_Article(Apt_Code, Aid, cnn.Repair_Article_Code);
                        await Save_Prive_View(cnn.Repair_Cost_Code.ToString());
                        //cnn.Repair_All_Cost = cnn.Repair_All_Cost + Convert.ToInt64(intAll_Cost);
                    ...
                    #endregion
                    //rae = await article_Lib.GetList_Repair_Article_Cycle_Sort(Aid, "Sort_A_Code", strSortA, Apt_Code);
                }
                else
                {
        ...
        /// 수선금액 삭제
        /// </summary>
        private async Task btnRemoveCost(Cost_Entity at)
        {
            bool isDelete = await JSRuntime.InvokeAsync<bool>("confirm", $"{Aid}번 글을 정말로 삭제하시겠습니까?");
            if (isDelete)
            {
                int a = Convert.ToInt32(at.Repair_Article_Code);
                await cost_Lib.Remove_Repair_Cost(at.Repair_Cost_Code);
                await article_Lib.Update_Cost_Count_Minus(a);
                await price_Set_Lib.Remove_CostCode(at.Repair_Cost_Code.ToString());
                await DetailsView();
            }
        }
        ...
        private async Task OnByRemoveCostA(Price_Set_Entity Cde)
        {
            ...
            if (isDelete)
            {
                ...
                pseList = await price_Set_Lib.GetList_A(cnn.Repair_Cost_Code.ToString());//GetList_A(cnn.Repair_Article_Code);
            }
        }
        ...
        private async Task Cost_Save()
        {
            ...
            enn = await cost_Lib.Detail_Cost(Apt_Code, cnn.Repair_Cost_Code.ToString());
            pseList = await price_Set_Lib.GetList_A(cnn.Repair_Cost_Code.ToString());
        }
