...
namespace Plan_Blazor_Lib
{
    ...
    public interface IArticle_Lib
    {
        ...
        /// <summary>
        /// 해당 수선항목 명으로 존재 여부 확인
        /// </summary>
        Task<int> Be_ArticleName(string Apt_Code, string Repair_Article_Name);

        /// <summary>
        /// 입력된 수선항목 수
        /// </summary>
        Task<int> Article_Count(string Repair_Plan_Code);

        /// <summary>
        /// 수선항목 이전 리스트
        /// </summary>
       Task<List<Article_Entity>> GetList_Article_Ago(string Apt_Code, string Now_Code, string Ago_Code);
    ...
    public class Article_Lib : IArticle_Lib
    {
        ...
        /// 수선항목 이전 리스트
        /// </summary>
        public async Task<List<Article_Entity>> GetList_Article_Ago(string Apt_Code, string Now_Code, string Ago_Code)
        {
            string sql = "Select Aid, Apt_Code, Repair_Plan_Code, Sort_A_Code, Sort_B_Code, Sort_C_Code, Sort_A_Name, Sort_B_Name, Sort_C_Name, Repair_Article_Name, Division, Amount, Unit, All_Cycle, Part_Cycle, Repair_Rate, Installation, Installation_Part, Repair_Article_Etc, User_ID, Cycle_Count, Cost_Count From Repair_Article as a Where Apt_Code = @Apt_Code And Repair_Plan_Code = @Ago_Code and del = 'A' And Repair_Article_Name = (Select Repair_Article_Name From Repair_Article Where Apt_Code = @Apt_Code And Repair_Plan_Code = @Now_Code And Repair_Article_Name = a.Repair_Article_Name) Order By Sort_A_Code Asc;";
            using (var connection = new SqlConnection(_db.GetConnectionString("Khmais_db_Connection")))
            {
                connection.Open();
                var aa = await connection.QueryAsync<Article_Entity>(sql, new { Apt_Code, Now_Code, Ago_Code }, commandType: CommandType.Text);
                return aa.ToList();
            }
        }
        ...
        /// 수선항목 이전 정보
        /// </summary>
        public Article_Entity Article_Ago(string Apt_Code, string Ago_Code, string Article_Name)
        {
            string sql = "Select Aid, Apt_Code, Repair_Plan_Code, Sort_A_Code, Sort_B_Code, Sort_C_Code, Sort_A_Name, Sort_B_Name, Sort_C_Name, Repair_Article_Name, Division, Amount, Unit, All_Cycle, Part_Cycle, Repair_Rate, Installation, Installation_Part, Repair_Article_Etc, User_ID, Cycle_Count, Cost_Count From Repair_Article Where Apt_Code = @Apt_Code And Repair_Plan_Code = @Ago_Code and del = 'A' And Repair_Article_Name = @Article_Name Order By Sort_A_Code Asc;";
            using (var connection = new SqlConnection(_db.GetConnectionString("Khmais_db_Connection")))
            {
                connection.Open();
                return connection.QuerySingleOrDefault<Article_Entity>(sql, new { Apt_Code, Ago_Code, Article_Name });                
            }
        }
        ...
        /// 해당 수선항목 명으로 존재 여부 확인
        /// </summary>
        public async Task<int> Be_ArticleName(string Apt_Code, string Repair_Article_Name)
        {
            string sql = "Select Count(*) From Repair_Article Where Apt_Code = @Apt_Code And Repair_Article_Name = @Repair_Article_Name";
            using (var conn = new SqlConnection(_db.GetConnectionString("Khmais_db_Connection")))
            {
                conn.Open();
                return await conn.QuerySingleOrDefaultAsync<int>(sql, new {Apt_Code, Repair_Article_Name });
            }
        }
        ...
        /// 입력된 수선항목 수
        /// </summary>
        public async Task<int> Article_Count(string Repair_Plan_Code)
        {
            string sql = "Select Count(*) From Repair_Article Where Repair_Plan_Code = @Repair_Plan_Code";
            using (var conn = new SqlConnection(_db.GetConnectionString("Khmais_db_Connection")))
            {
                return await conn.QuerySingleOrDefaultAsync<int>(sql, new { Repair_Plan_Code }, commandType: CommandType.Text);
            }
        }
    }
}